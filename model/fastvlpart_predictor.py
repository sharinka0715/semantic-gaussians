...
class FastVLPart:
    ...
    def extract_image_feature(self, img_dir, img_size=None):
        ...
        if len(filter_boxes) > 0:
            ...
            prompt_process = FastSAMPrompt(str(img_dir), results, device="cuda")
            masks = prompt_process.box_prompt(bboxes=boxes_filter.numpy().tolist()).astype(np.bool_)

            # remove small disconnected regions and holes
            fine_masks = []
            for mask in masks:  # masks: [num_masks, h, w]
                fine_masks.append(remove_small_regions(mask, 400, mode="holes")[0])
