...
class FastSAMCLIP:
    ...
    def extract_image_feature(self, img_dir, img_size):
        ...
        results = self.sam(str(img_dir), device="cuda", retina_masks=True, imgsz=img_size, verbose=False)
        ...
        for t in range(boxes.shape[0]):
            ...
            h_thresh = int(img_size[0] * 0.1)
            w_thresh = int(img_size[1] * 0.1)
            if x2 < w_thresh or x1 > img_size[1] - w_thresh or y2 < h_thresh or y1 > img_size[0] - h_thresh:
                continue
        ...
        features = torch.zeros((768, img_size[0], img_size[1]), dtype=torch.half)
