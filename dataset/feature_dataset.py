...
class MVImgNetDataset(FeatureDataset):
    # Augmentation arguments
    SCALE_AUGMENTATION_BOUND = (0.5, 1.5)
    ROTATION_AUGMENTATION_BOUND = (
        (-np.pi, np.pi),
        (-np.pi, np.pi),
        (-np.pi, np.pi),
    )
    TRANSLATION_AUGMENTATION_RATIO_BOUND = ((-5, 5), (-5, 5), (-5, 5))
    ELASTIC_DISTORT_PARAMS = ((0.2, 0.4), (0.8, 1.6))
    ...
    def __init__(
        ...
        for cls in self.classes:
            scenes = os.listdir(os.path.join(gaussians_dir, cls))
            scenes.sort()
            for scene in scenes:
                features = os.listdir(os.path.join(point_dir, cls, scene))
                features.sort()
                for feature in features:
                    ply_path = os.path.join(
                        gaussians_dir,
                    ...
                    feature_path = os.path.join(point_dir, cls, scene, feature)
                    self.data.append([ply_path, feature_path, 0])
