...
def confusion_matrix(pred_ids, gt_ids, num_classes):
    ...
    if NO_FEATURE_ID in pred_ids:  # some points have no feature assigned for prediction
        ...
        return confusion[:, :num_classes]  # confusion[:num_classes, :num_classes]
    ...
    # return (
    #     np.bincount(pred_ids[idxs] * num_classes + gt_ids[idxs], minlength=num_classes**2)
...
def evaluate_confusion(confusion, stdout=False, dataset="scannet_20"):
    if stdout:
        print("evaluating", confusion.sum(), "points...")

    if "scannet_20" == dataset:
        CLASS_LABELS = SCANNET_CLASS_LABELS_20
    elif "scannet_200" == dataset:
        CLASS_LABELS = SCANNET_CLASS_LABELS_200
    N_CLASSES = len(CLASS_LABELS)
